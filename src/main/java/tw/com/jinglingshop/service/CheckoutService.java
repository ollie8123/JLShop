...
public class CheckoutService {
   ...
   private CreditcardRepository creditcardRepository;
    ...
    @Transactional
    //新增訂單
    public synchronized  String generateOrders(JSONObject object,String userEmail) throws Exception {

       //ArrayList<List<HashMap<String,Object>>> EmailArray = new ArrayList<>();
        //email賣家資訊陣列
        List<HashMap<String,Object>> emailMap = null;
        int error= 0;
        String msg= null;
        try {
            emailMap = new ArrayList<>();
            error = 0;
            msg = "商品變動請重試";
            ...
            for(int i=0;i<sellerOrderList.length();i++){
                ...
                //初始化訂單狀態
                order.setOrderStatus(orderStatusService.findOrderStatusById(5));
                   ...
                   int count =productJSONArray.getJSONArray(j).getInt(1);
                    ...
                    if(shoppingCart!=null){
                        ...
                        if(shoppingCart.getProduct().getStocks()>=count){
                            ...
                            shoppingCart.getProduct().setSales((shoppingCart.getProduct().getSales()+count));
                            //扣除購物車數量，如果扣除後為0則刪除
                    ...
                    if(coupon.getAvailableNumber()>coupon.getUsed()){
                        ...
                        if(coupon.getDiscountRate()!=null){
                         ...
                         //一般折扣
                            ...
                            sellerEmailMap.put("discountPrice",(int)coupon.getDiscountAmount());
                        }
                        //根據couponId以及使用者找出優惠券明細
